...
```
The verification code that appears at the end of the URL after authorization in browser should be input as verification PIN. If the parameter store is enabled - AuthZot(cache=TRUE) - zoteroR stores the credentials in a local file called "ZoteroCredentials.rds", which should be considered confidential since it provides access to a given Zotero account. If a pre-existing "ZoteroCredentials.rds" exists, it is loaded automatically.
...
If you enable `cache=TRUE` it will store a csv file in the `zot_csv_templates` subf-folder of the current working directory.
...
Zotero does not store data in a tabular format, so some transformation will be necessary.
